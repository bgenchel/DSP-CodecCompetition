...
if strcmp(method, 'default_default') % default bit allocation
...
if strcmp(method, 'default_uniform') % uniformly allocate to bands in a frame
    bits_average = floor(((bitrate/Fs) * (N/2)) - (scalebits * num_subbands));
    bits_frame = ones(1, frame_num) * bits_average;
    bitsleft = 0;
    for frame_count=1:frame_num
...
if strcmp(method, 'default_var') % uniformly allocate to bands in a frame
    bits_average = floor(((bitrate/Fs) * (N/2)) - (scalebits * num_subbands));
    bits_frame = ones(1, frame_num) * bits_average;
    bitsleft = 0;
    for frame_count=1:frame_num
...
% not good
if strcmp(method, 'energy_default') % assign by log energy across time, default/uniform within frame
...
if strcmp(method, 'maxeng_default')
...
if strcmp(method, 'sqrtstd_default')
    t = sqrt(std(y, 0, 2));
    p = t./sum(t);
    bits_frame = floor(p * total_bits);
    for frame_count=1:frame_num
...
if strcmp(method, 'sqrtstd_uniform')
    t = sqrt(std(y, 0, 2));
    p = t./sum(t);
    bits_frame = floor(p * total_bits);
    for frame_count=1:frame_num
